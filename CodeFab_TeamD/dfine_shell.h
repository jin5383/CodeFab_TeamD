...
// Environment(변수 컨텍스트)와 ImportScope(import 컨텍스트)를 인스턴스가 직접 들고 있어서,
// 여러 줄에 걸쳐 세션 전체의 컨텍스트(이전 줄에서 선언한 변수/임포트한 모듈)를 기억한다.
//
// [디자인 패턴 사용처] 이 클래스는 Interpreter(Facade, interpreter.h 참고)의 대표적인
// 클라이언트다. runLine()에서 Assembler/Checker/Executor를 직접 조합하지 않고
// `Interpreter(output).run(line, environment, imports)` 한 줄만 호출하는 것에 주목할 것.
