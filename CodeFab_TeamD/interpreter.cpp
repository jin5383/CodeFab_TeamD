...
void Interpreter::run(const std::string& source, Environment& environment, ImportScope& imports) const
{
	...
	if (checker.check(program) == CheckerErrno::success)
		executor.execute(program, environment, imports);
}

void Interpreter::run(const std::string& source, Environment& environment) const
{
	ImportScope imports;
	run(source, environment, imports);
}
