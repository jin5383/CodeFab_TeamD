...
> Unit 계약 근거: `docs/design/unit-io-spec.md` (Assembler/Checker/Executor Input·Output 계약),
> `docs/design/program-tree-struct-spec.md` (Program/Expr/Stmt 트리 구조)
...
- **Checker Unit / Executor Unit**: Input은 Assembler를 실제로 호출해서 얻지 않고, **Assembler
  Unit의 output과 동일한 포맷**(`docs/design/program-tree-struct-spec.md`에 정의된 `Program`/`Expr`/
