...
- 배열 값 자체: `std::shared_ptr<std::vector<LiteralValue>>`를 `LiteralValue` variant에 추가(3.7절에서 Lee/Park과
  함께 최종 확정)
- `Array(3)`는 문법상 `CallExpr`(callee가 예약된 이름 `Array`)로 파싱해도 되고, 전용 `ArrayExpr : Expr { Expr* size; }`로
  별도 파싱해도 된다 — **Park의 인스턴스 생성 문법(`Robot()`)과 겹치지 않게, `Array`는 파서에서 예약어로 취급해
  `ArrayExpr`로 분기하는 쪽을 권장**(클래스 인스턴스화와 혼동 방지, Checker에서 "Array를 클래스 이름으로 재선언"하는
  것도 자동으로 막힘).

**Checker 규칙**
- 배열 생성 시 크기가 숫자가 아님 → 에러(단, `Array(a)`처럼 변수를 통한 크기 지정은 런타임에만 값을 알 수 있으므로
  이 역시 실질적으로 Executor 런타임 에러일 가능성이 높음 — 3.1.1과 동일 패턴으로 확인 필요)

**Executor 규칙**
- 범위를 벗어난 인덱스 접근, 인덱스가 숫자가 아님, 배열이 아닌 대상에 `[]` 사용 → 런타임 에러

**확인 필요 항목 (3.3.1)**
- 배열 크기/인덱스가 정수가 아닌 소수(예: `arr[1.5]`)일 때 에러로 볼지 내림 처리할지 원문에 명시 없음 → 확인 필요.
