...
**인스턴스 생성 규칙 (p.9 확정)**
- `ClassName(args...)`로 호출 시, 클래스에 `init` 메서드가 **있으면** 주어진 인자와 함께 `init`을 호출해 인스턴스를
  초기화한다(3.2 문법 예시와 동일).
- 클래스에 `init`이 **없으면** `ClassName(args...)` 호출은 인자가 없는 일반 함수 호출과 동일하게 취급되어, 빈
  인스턴스만 생성한다. 이때 인자를 하나라도 넘기면(예: `init`이 없는 `Robot`에 `Robot(1, 2)`) **인자 개수 불일치
  에러**로 처리한다(함수의 인자 개수 불일치 규칙, 3.1절과 동일한 에러 종류 — "기대 인자 0개, 전달 인자 N개").
...
- `init`이 없는 클래스를 인자와 함께 호출(위 "인스턴스 생성 규칙" 참고)하는 경우의 인자 개수 불일치는, 함수 호출의
  인자 개수 검사와 마찬가지로 대상이 실제로 인자 없는 인스턴스 생성인지 정적으로 알 수 있는 경우에만 Checker가
  잡고, 그렇지 않으면(런타임에만 콜리(callee)의 실제 타입을 아는 경우) 3.1.1과 동일하게 Executor 런타임 에러가 된다.
...
- 필드 **쓰기**(`r.field = value`, `This.field = value` 모두 동일)는 그 필드가 클래스에 선언되어 있지 않아도 항상
  성공하며, 그 시점에 인스턴스의 필드로 새로 생성된다(p.10, p.11 확정 — `This`를 통한 접근도 외부에서 `r.field =
  value`로 쓸 때와 동일한 동적 필드 생성 규칙을 따른다).
- 필드/메서드 **읽기**는 그 이름이 인스턴스(및 상속 체인)에 없으면 런타임 에러. 인스턴스가 아닌 대상에 필드
  접근(읽기/쓰기 모두)하는 것도 런타임 에러.
...
**watch 동작 규칙 (p.35 확정)**: `watch`는 특정 변수 슬롯을 고정 참조하는 것이 아니라, **매 정지 시점마다 그 이름을
현재 스코프 체인에서 다시 조회**하는 방식으로 구현한다.
- 조회 시점에 그 이름이 현재 스코프 체인에서 찾아지면 값을 출력한다.
- 찾아지지 않으면(감시 등록 당시 지역변수였고 이후 그 지역을 벗어난 경우 등) `"값을 참조할 수 없습니다"`를 출력한다.
- `unwatch`로 명시적으로 제거하기 전까지는 목록에서 빠지지 않는다. 예를 들어 `for` 반복문 본문이 블록(`{ }`)이면
  현재 구현(`Executor_Unit.cpp`의 `BlockStmt` 처리)은 반복마다 새 `Environment`를 만들어 블록을 실행하므로, 그
  본문 안의 지역변수를 감시하면 매 반복마다 "블록을 벗어나며 사라짐 → 다음 반복에서 같은 이름으로 다시 선언됨"이
  반복된다. 이름 기준 재조회 방식이면 이 경우 별도 처리 없이도 반복마다 자연스럽게 값이 다시 보인다.
