...
> 근거: `docs/additional-requirement/additional-requirement-impl-spec.md` 3.1절, `docs/additional-requirement/phase0-review-checklist.md`(Lee 항목),
...
- Phase 0(`a70cde9`)는 이미 `additional_requirement` 브랜치에 병합됨. `ast.h`(FUNC/RETURN/COMMA
  ...
  스텁도 이미 자리잡혀 있다.
- `docs/additional-requirement/phase0-review-checklist.md`의 Lee 항목은 권장안(A)으로 확정:
...
`docs/additional-requirement/scenarios/lee-function-scenarios.md`를 별도 파일로 새로 만들어 함수 기능 전용 시나리오를
문서화해야 한다(다음 커밋에서 진행):
...
나오도록 Green으로 만든다. `docs/additional-requirement/scenarios/lee-function-scenarios.md`의 에러 시나리오에도 이
두 케이스를 추가한다.
...
Red 테스트 3개 추가 후 Green 확인, `docs/additional-requirement/scenarios/lee-function-scenarios.md`에도 두 케이스
반영.
