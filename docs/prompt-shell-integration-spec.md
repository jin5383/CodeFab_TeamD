...
한 줄이 들어올 때마다 버퍼에 줄을 추가하고, 아래 조건을 만족할 때만 파이프라인을 실행한다.

- 문자열 리터럴(`"..."`) 내부가 아닌 위치에서 `{`/`(` 개수와 `}`/`)` 개수를 각각 누적 카운트한다.
- `괄호 깊이(paren depth) == 0` 이고 `중괄호 깊이(brace depth) == 0` 일 때만 "완전한 입력"으로
  간주하고 파이프라인을 실행한 뒤 버퍼를 비운다.
- 두 깊이 중 하나라도 0이 아니면 버퍼를 비우지 않고 다음 줄을 계속 받는다(프롬프트를
  `... ` 같은 continuation 표시로 바꿀 수 있으나 필수는 아니다).
- 이 규칙은 문자 단위 괄호/중괄호 균형만 본다. 세미콜론 누락처럼 균형은 맞지만 불완전한
  구문(`var a = 3` 세미콜론 없이 줄 끝)은 Assembler가 4절의 구문 오류 계약에 따라 오류로
  보고한다 — PromptShell이 세미콜론 유무까지 추적하지 않는다(YAGNI, `테스트 스크립트.md`의
  모든 정상 시나리오는 문장이 세미콜론으로 끝나거나 `}`로 끝난다).

예시 (`테스트 스크립트.md` 2)절 중첩 스코프 시나리오):

```
var outer = "A";        입력 즉시 실행 (depth 0)
{                        depth 1, 버퍼링 시작
  var inner = "B";       depth 1, 계속 버퍼링
  {                      depth 2, 계속 버퍼링
    print outer + inner; depth 2, 계속 버퍼링
  }                      depth 1, 계속 버퍼링
}                        depth 0 → 버퍼 전체를 하나의 source로 assemble → check → execute
```
...
{
public:
    ExecutorSession();                      // global Environment를 세션 동안 유지
    void run(const Program& program);
private:
    /* 기존 Environment global 상태 */
};
```

- `checkAssembly`/`executeAssembly` 내부 구현은 각각 `CheckerSession`/`ExecutorSession`을
  1회용으로 생성해 위임하는 형태로 리팩토링한다(로직 중복 방지).
- PromptShell은 세션 시작 시 `CheckerSession`, `ExecutorSession`을 각 1개씩 생성하고, 완전한
  입력이 모일 때마다 같은 세션 인스턴스에 `check`/`run`을 반복 호출한다.

### 2.4 PromptShell 메인 루프 (의사 코드)
...
while (std::getline(std::cin, line))
{
    ...
    if (braceDepth == 0 && parenDepth == 0 && !isBlank(buffer))
    {
...
2. **Line-by-line 모드**: 코드 블록을 줄 단위로 쪼개 2.4절의 버퍼링 규칙대로 `CheckerSession`/
   `ExecutorSession`에 순서대로 흘려보낸다. 이 모드가 실패하면 "한 줄씩 입력해도 상태를
...
`stdout` 캡처는 `std::cout`의 `rdbuf()`를 `std::ostringstream`으로 교체했다가 복원하는 방식을
쓴다(추가 의존성 없음).
...
2. `Executor_Unit.cpp`: `Environment global`을 `ExecutorSession`으로 노출, `executeAssembly`는
   내부적으로 1회용 `ExecutorSession`에 위임하도록 리팩토링.
3. `function.h`: `CheckerSession`/`ExecutorSession` 선언 추가.
4. PromptShell 진입점 구현 (2.4절 의사 코드 기준). `main.cpp`는 현재 gtest 러너 전용이므로,
   별도 실행 파일/모드로 분리할지 여부는 빌드 구성(`CodeFab_TeamD.vcxproj`)과 함께 별도 결정.
5. `test_integrationcpp`: 3절 설계에 따른 Integration Test 작성.
6. Assembler 구문 오류 계약 상세 설계 및 구현 (4절 최소 계약 기준).
